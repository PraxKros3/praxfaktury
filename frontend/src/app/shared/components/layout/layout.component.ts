...
export class LayoutComponent {
  ...
  navItems = [
    ...
    { path: '/manual-entries', label: 'Zákazky', icon: 'clipboard' },
    { path: '/invoices', label: 'Faktúry', icon: 'file-text' },
    { path: '/clients', label: 'Klienti', icon: 'users' },
    { path: '/projects', label: 'Projekty', icon: 'folder' },
    { path: '/settings', label: 'Nastavenia', icon: 'settings' },
  ];
  ...
  syncToggl() {
    ...
    this.api.syncToggl().subscribe({
      next: (res) => {
        this.syncMessage.set(`✓ ${res.clients} klientov, ${res.projects} projektov, ${res.timeEntries} záznamov`);
        this.syncing.set(false);
      },
      error: (err) => {
        const msg = err?.error?.message || 'Chyba pri synchronizácii';
